...
int comp_pair (const void *left, const void *right) {
    float da = ((Pair *)left)->distance;
    float db = ((Pair *)right)->distance;
    if (da < db) return -1;
...
int main() {
    ...
    for(int k =0;k<pairs.size;k++){
        ...
        printf("a.x: %ld b.x %ld after distance: %f\n", ab->a.x, ab->b.x, ab->distance);
    }
    ...
    for(int i =0;i<10;i++){ 
        ...
        Pair *s = (Pair*)vector_get(&pairs,i);
    ...
    // 3 largest
    int sum = (*(Vector **)vector_get(&grid, 0))->size;
    for (int i = 1; i < 3; i++) {
        sum *= (*(Vector **)vector_get(&grid, i))->size;
    }
    printf("sum %d\n", sum);
